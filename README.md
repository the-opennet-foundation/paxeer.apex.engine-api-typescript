...
const response: PaxeerApexEngineAPI.HealthCheckResponse = await client.health.check();
```
...
const response = await client.health.check().catch(async (err) => {
  if (err instanceof PaxeerApexEngineAPI.APIError) {
...
1. Via the `PAXEER_APEX_ENGINE_API_LOG` environment variable
...
const client = new PaxeerApexEngineAPI({
  logger: logger.child({ name: 'PaxeerApexEngineAPI' }),
